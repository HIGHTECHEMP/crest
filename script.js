...
const CARD_FEE_EUR = 200.00;
let selectedCardType = 'VISA';
let activeCardRequestId = null;
...
function injectLoader() {
  ...
  style.textContent = `
    ...
    @keyframes crestLogoPulse {
      0%, 100% { transform: scale(1) translateY(0); filter: drop-shadow(0 0 0px rgba(37,99,235,0)); }
      40% { transform: scale(1.12) translateY(-6px); filter: drop-shadow(0 8px 24px rgba(37,99,235,0.35)); }
      60% { transform: scale(1.08) translateY(-3px); filter: drop-shadow(0 4px 16px rgba(37,99,235,0.25)); }
    }
    @keyframes crestLogoRing {
  ...
  div.id = 'crest-loader';
  div.style.cssText = 'display:none;position:fixed;inset:0;z-index:999999;background:rgba(15,23,42,0.55);backdrop-filter:blur(5px);align-items:center;justify-content:center;flex-direction:column;gap:20px;animation:crestFadeIn 0.2s ease;';
  div.innerHTML = `
    <div style="position:relative;width:80px;height:80px;display:flex;align-items:center;justify-content:center;">
      <div style="position:absolute;inset:0;border:2px solid var(--primary);border-radius:50%;animation:crestLogoRing 1.6s ease-out infinite;"></div>
      <div style="position:absolute;inset:0;border:2px solid var(--primary);border-radius:50%;animation:crestLogoRing 1.6s ease-out infinite 0.8s;"></div>
      <img src="crest-logo.jpeg" alt="Crest" style="width:56px;height:56px;object-fit:contain;border-radius:50%;animation:crestLogoPulse 1.6s ease-in-out infinite;position:relative;z-index:2;">
    </div>
    <p id="crest-loader-text" style="color:white;font-size:14px;font-weight:600;letter-spacing:0.5px;text-shadow:0 2px 8px rgba(0,0,0,0.3);margin:0;">Please wait...</p>
  `;
...
async function showPage(page) {
  ...
  // Dynamic browser tab title
    ...
    txdetail: 'Transaction Details | Crest'
  };
  ...
  if (!appState.isAdmin) {
    ...
    if (page === 'wallet') await renderWallet();
...
function renderTxItem(t) {
  ...
  return `
    <div class="transaction-item" onclick='openTransactionDetail(${JSON.stringify(t).replace(/'/g, "&#39;")})' style="cursor:pointer;">
...
/* -------------------- FEATURE LOCKS -------------------- */
async function getFeatureLocks(userId) {
  const locks = { paybills: false, sendmoney: false, withdraw: false };
  if (!userId) return locks;
  const { data } = await sb.from('feature_locks').select('*').eq('user_id', userId);
...
async function checkLockAndOpen(page) {
  if (!currentProfile) return;
  const locks = await getFeatureLocks(currentProfile.id);
  if (locks[page]) {
...
async function handlePayBill(e) {
  ...
  await withLoader(async () => {
    ...
    currentProfile.balance = newBal;
      ...
      details: { category, reference: ref, reason: desc }
    });
  }, 'Processing payment...');
...
async function handleSendMoney(e) {
  ...
  await withLoader(async () => {
    ...
    currentProfile.balance = newBal;
      ...
      details: { recipient_name: name, bank, account, note }
    });
  }, 'Sending money...');
...
async function reviewWithdrawal(e) {
  e.preventDefault();
  if (!currentProfile) return;

  if (!currentProfile.withdrawal_pin) {
    showToast('Please set a withdrawal PIN in Account \u2192 Security first', 'error');
    showPage('account');
    return;
  }
  ...
  withdrawDraft = { name, account, code, bank, amount, reason };

  document.getElementById('confirmWithdrawName').textContent = name;
  document.getElementById('confirmWithdrawAccount').textContent = account;
  document.getElementById('confirmWithdrawCode').textContent = code;
  document.getElementById('confirmWithdrawBank').textContent = bank;
  document.getElementById('confirmWithdrawReason').textContent = reason || '\u2014';
  document.getElementById('confirmWithdrawAmount').textContent = fmtMoney(amount, currentProfile.currency);

  document.getElementById('withdrawFormCard').style.display = 'none';
  document.getElementById('withdrawConfirmCard').style.display = 'block';
  document.getElementById('withdrawPin').value = '';
  setTimeout(() => document.getElementById('withdrawPin').focus(), 100);
}

function cancelWithdrawal() {
  document.getElementById('withdrawConfirmCard').style.display = 'none';
...
async function confirmWithdrawal() {
  ...
  if (!pin) return showToast('Please enter your withdrawal PIN', 'error');
  if (pin !== currentProfile.withdrawal_pin) {
    document.getElementById('withdrawPin').value = '';
    return showToast('Incorrect PIN. Please try again.', 'error');
  }

  /* FIX: include 'code' in the destructuring */
  const { name, account, code, bank, amount, reason } = withdrawDraft;
  ...
  await withLoader(async () => {
    ...
    /* FIX: add 'code' to the details object */
    await addTransaction({
      id: genId('tx'),
      type: 'expense',
      title: 'Withdrawal to ' + name,
      subtitle: 'SEPA \u2022 ' + bank + ' \u2022 ' + account,
      amount,
      date: todayStr(),
      status: 'completed',
      icon_class: 'red',
      amount_class: 'expense-amount',
      details: {
        recipient_name: name,
        bank,
        account,
        code: code,        // <-- BIC now saved
        method: 'SEPA',
        reason
      }
    });
  ...
  cancelWithdrawal();
  showToast('Withdrawal sent successfully', 'success');
  showPage('home');
}
...
async function confirmCryptoDeposit() {
  ...
  const file   = document.getElementById('cryptoProof').files[0];
  if (!amount||amount<=0) return showToast('Please enter a valid amount','error');
  if (file && file.size > 5 * 1024 * 1024) return showToast('File must be under 5MB','error');

  const info = CRYPTO_INFO[coin];
  const depId = genId('dep');

  if (activeCardRequestId) {
    await withLoader(async () => {
      const { data: request, error: reqErr } = await sb.from('card_requests').select('id,user_id,status,fee_amount,fee_currency').eq('id', activeCardRequestId).eq('user_id', currentProfile.id).single();
      if (reqErr || !request) throw new Error('Card request not found');
      if (request.status !== 'awaiting_payment') throw new Error('This card request is no longer awaiting payment');
      if (Number(request.fee_amount) !== CARD_FEE_EUR || request.fee_currency !== 'EUR') throw new Error('Invalid card fee');

      let proofUrl = null;
      if (!file) throw new Error('Proof of payment is required for the card fee');
      const ext = (file.name.split('.').pop() || 'jpg').toLowerCase();
      const path = `proofs/${currentProfile.id}/card-${activeCardRequestId}.${ext}`;
      const { error: upErr } = await sb.storage.from('proofs').upload(path, file, { upsert: true, contentType: file.type });
      if (upErr) throw new Error(upErr.message);
      proofUrl = sb.storage.from('proofs').getPublicUrl(path).data?.publicUrl || null;

      const { error: depErr } = await sb.from('admin_deposits').insert([{
        id: depId, user_id: currentProfile.id, coin: 'CARD FEE (EUR)', amount: CARD_FEE_EUR,
        txid: txid || 'CARD-FEE', proof_url: proofUrl, status: 'processing',
        purpose: 'card_fee', card_request_id: activeCardRequestId
      }]);
      if (depErr) throw depErr;


      await addTransaction({
        id: depId, type:'expense', title:'Virtual Card Issuance Fee',
        subtitle: 'Fixed €200 card fee • Pending admin review', amount: CARD_FEE_EUR, date: todayStr(),
        status:'processing', icon_class:'blue', amount_class:'pending-amount',
        details: { purpose:'card_fee', card_request_id: activeCardRequestId, card_type: 'Card issuance' }
      });
    }, 'Submitting card payment...');

    activeCardRequestId = null;
    document.getElementById('cryptoAmount').value = '';
    document.getElementById('cryptoTxid').value = '';
    document.getElementById('cryptoProof').value = '';
    resetDepositMode();
    showToast('Card payment submitted for admin review', 'success');
    showPage('wallet');
    return;
  }
  let proofUrl = null;
  ...
  await withLoader(async () => {
    ...
    if (file) {
      ...
      const path = `proofs/${currentProfile.id}/${depId}.${ext}`;
      const { error: upErr } = await sb.storage.from('proofs').upload(path, file, { upsert: true, contentType: file.type });
      if (upErr) throw new Error(upErr.message);
    ...
    await sb.from('admin_deposits').insert([{
      id: depId, user_id: currentProfile.id, coin: info.name, amount,
      txid: txid || 'N/A', proof_url: proofUrl, status: 'processing'
    }]);
    ...
    // 3. Add user transaction
      ...
      details: { coin: info.name, txid: txid || 'N/A', network: info.network }
    });
  }, 'Submitting deposit...');
...
function renderAccount() {
  ...
  if (pinStatus) {
    pinStatus.textContent = u.withdrawal_pin ? 'PIN is set' : 'Not set \u2014 required for withdrawals';
    pinStatus.style.color = u.withdrawal_pin ? 'var(--success)' : 'var(--warning)';
  }
  if (setPinBtn) {
    setPinBtn.textContent = u.withdrawal_pin ? 'Change' : 'Set PIN';
  }
}
...
async function renderAdminPanel() {
  ...
  // Per-user feature locks fetched dynamically in renderUserLockToggles
  ...
  container.innerHTML = `
    <section class="crypto-deposit-card">
      <h3 style="font-size:18px;font-weight:800;margin-bottom:16px;color:var(--text-primary)">Feature Locks (Per User)</h3>
      <div class="form-group" style="margin-bottom:16px">
        <label class="form-label">Select User</label>
        <select class="form-input" id="adminLockTargetUser" onchange="renderUserLockToggles(this.value)">
          <option value="">-- Select a user --</option>
          ${(allUsers||[]).map(u=>`<option value="${u.id}">${u.name} (${u.email})</option>`).join('')}
        </select>
      </div>
      <div id="userLocksContainer" style="display:none;flex-direction:column;gap:12px"></div>
    </section>
    ...
    <section class="crypto-deposit-card">
      <h3 style="font-size:18px;font-weight:800;margin-bottom:16px;color:var(--text-primary)">Virtual Card Requests</h3>
      <div style="display:flex;flex-direction:column;gap:10px">
        ${(() => {
          const cardFees = (deposits||[]).filter(d=>d.purpose==='card_fee' && d.status==='processing');
          return cardFees.length ? cardFees.map(d=>`
            <div style="padding:14px;background:var(--bg);border-radius:12px;border:1px solid var(--border)">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                <span style="font-weight:700;color:var(--text-primary)">${d.profiles?.name||'User'} — €200.00 card fee</span>
                <span class="transaction-status processing">Payment Review</span>
              </div>
              <p style="font-size:12px;color:var(--text-secondary);margin-bottom:10px">${d.profiles?.email||''} • ${new Date(d.created_at).toLocaleString()} ${d.proof_url ? `• <a href="${d.proof_url}" target="_blank" style="color:var(--primary);font-weight:600;">View Proof</a>` : ''}</p>
              <div style="display:flex;gap:8px">
                <button class="security-item-btn" onclick="updateDepositStatus('${d.id}','confirmed')">Approve & Issue Card</button>
            ...
            </div>`).join('') : '<p style="color:var(--text-muted);font-size:14px">No pending card requests.</p>';
        })()}
      </div>
    </section>
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${!(deposits||[]).filter(d=>d.status==='processing').length
          ...
          : deposits.filter(d=>d.status==='processing').map(d=>`
            <div style="padding:14px;background:var(--bg);border-radius:12px;border:1px solid var(--border)">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                <span style="font-weight:700;color:var(--text-primary)">${d.profiles?.name||'User'} — ${d.purpose==='card_fee' ? 'Card Fee €200.00' : d.coin+' '+d.amount}</span>
                <span class="transaction-status processing">Processing</span>
              </div>
              <p style="font-size:12px;color:var(--text-secondary);margin-bottom:10px">${d.purpose==='card_fee' ? 'Card issuance payment' : 'TXID: '+d.txid} • ${new Date(d.created_at).toLocaleString()} ${d.proof_url ? `• <a href="${d.proof_url}" target="_blank" style="color:var(--primary);font-weight:600;">View Proof</a>` : ''}</p>
              <div style="display:flex;gap:8px">
...
function renderLockRowHTML(key,label,locked,userId) {
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px;background:var(--bg);border-radius:10px;border:1px solid var(--border)">
      ...
      <label class="toggle-switch">
        <input type="checkbox" ${locked?'checked':''} onchange="toggleFeatureLock('${key}', '${userId}', this)">
        <span class="toggle-slider"></span>
      </label>
    </div>`;
}

async function renderUserLockToggles(userId) {
  const container = document.getElementById('userLocksContainer');
  if (!container || !userId) { if (container) container.style.display = 'none'; return; }
  const locks = await getFeatureLocks(userId);
  container.innerHTML = `
    <div style="margin-bottom:8px;font-size:13px;color:var(--text-muted);font-weight:500">Managing locks for selected user</div>
    ${renderLockRowHTML('paybills', 'Pay Bills', locks['paybills'], userId)}
    ${renderLockRowHTML('sendmoney', 'Send Money', locks['sendmoney'], userId)}
    ${renderLockRowHTML('withdraw', 'Withdraw', locks['withdraw'], userId)}
  `;
  container.style.display = 'flex';
}

async function toggleFeatureLock(feature, userId, checkbox) {
  if (!userId) {
  ...
  try {
    await withLoader(async () => {
      const { data, error } = await sb.from('feature_locks')
        .select('*').eq('feature_name', feature).eq('user_id', userId).maybeSingle();
      if (error) throw error;
      if (data) {
        await sb.from('feature_locks')
          .update({ locked: desiredState, updated_at: new Date().toISOString() })
          .eq('feature_name', feature).eq('user_id', userId);
      } else {
        await sb.from('feature_locks').insert([{
          feature_name: feature, user_id: userId, locked: desiredState,
          updated_at: new Date().toISOString()
        }]);
      }
    }, 'Updating...');
    showToast(feature + ' is now ' + (desiredState ? 'LOCKED' : 'UNLOCKED') + ' for this user',
      desiredState ? 'warning' : 'success');
  } catch (err) {
    checkbox.checked = !desiredState;
    showToast('Failed to update lock: ' + err.message, 'error');
    console.error(err);
  }
}
...
async function updateDepositStatus(id, status) {
  await withLoader(async () => {
    ...
    if (dep) {
      ...
      if (dep.purpose === 'card_fee' && dep.card_request_id && status === 'rejected') {
        await sb.from('card_requests').update({ status: 'rejected' }).eq('id', dep.card_request_id);
      }
      if (status === 'confirmed' && dep.purpose !== 'card_fee') {
...
function openSetPinModal() {
  const hasPin = !!(currentProfile && currentProfile.withdrawal_pin);
  document.getElementById('pinModalTitle').textContent = hasPin ? 'Change Withdrawal PIN' : 'Set Withdrawal PIN';
  document.getElementById('currentPinGroup').style.display = hasPin ? 'flex' : 'none';
  document.getElementById('currentPinInput').value = '';
  document.getElementById('newPinInput').value = '';
  document.getElementById('confirmPinInput').value = '';
  document.getElementById('pinSubmitBtn').textContent = hasPin ? 'Update PIN' : 'Set PIN';
  document.getElementById('setPinModal').classList.add('active');
}
function closeSetPinModal() { document.getElementById('setPinModal').classList.remove('active'); }
...
async function handleSetPin(e) {
  ...
  if (currentProfile.withdrawal_pin && current !== currentProfile.withdrawal_pin) {
  ...
  if (!/^\d+$/.test(newPin)) {
    return showToast('PIN must contain only numbers', 'error');
  }
  if (newPin !== confirm) {
  ...
  await withLoader(async () => {
    await sb.from('profiles').update({ withdrawal_pin: newPin }).eq('id', currentProfile.id);
  }, 'Saving PIN...');

  currentProfile.withdrawal_pin = newPin;
  closeSetPinModal();
  showToast('Withdrawal PIN updated', 'success');
  renderAccount();
}

/* -------------------- TRANSACTION DETAIL PAGE -------------------- */
let txDetailBackPage = 'transactions';

function openTransactionDetail(tx) {
  if (typeof tx === 'string') tx = JSON.parse(tx);
  txDetailBackPage = appState.currentPage;

  document.getElementById('txDetailTitlePage').textContent = tx.title || 'Transaction';
  document.getElementById('txDetailStatusPage').textContent = (tx.status || 'completed').toUpperCase();
  document.getElementById('txDetailStatusPage').style.color = tx.status==='processing' ? 'var(--warning)' : (tx.type==='income'?'var(--success)':'var(--danger)');
  document.getElementById('txDetailAmountPage').textContent = (tx.type==='income'?'+':'-') + fmtMoney(tx.amount, currentProfile?.currency||'EUR');
  document.getElementById('txDetailAmountPage').style.color = tx.type==='income'?'var(--success)':'var(--text-primary)';
  document.getElementById('txDetailIdPage').textContent = (tx.id || '').slice(0, 24) + '...';
  document.getElementById('txDetailDatePage').textContent = tx.date || '-';
  document.getElementById('txDetailTimePage').textContent = tx.created_at ? new Date(tx.created_at).toLocaleTimeString() : '-';

  const fields = document.getElementById('txDetailFieldsPage');
  let html = '';
  ...
  if (tx.details && typeof tx.details === 'object') {
    ...
    for (const [key, label] of Object.entries(detailMap)) {
      if (tx.details[key]) {
        html += `<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);"><span style="color:var(--text-secondary);font-size:14px;">${label}</span><span style="color:var(--text-primary);font-weight:600;font-size:14px;text-align:right;max-width:60%;word-break:break-all;">${tx.details[key]}</span></div>`;
      }
    }
  }

  if (!html) {
    html = `<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);"><span style="color:var(--text-secondary);font-size:14px;">Description</span><span style="color:var(--text-primary);font-weight:600;font-size:14px;text-align:right;max-width:60%;word-break:break-all;">${tx.subtitle || '-'}</span></div>`;
  }

  html += `<div style="display:flex;justify-content:space-between;padding:10px 0;"><span style="color:var(--text-secondary);font-size:14px;">Type</span><span style="color:var(--text-primary);font-weight:600;font-size:14px;text-transform:capitalize;">${tx.type || '-'}</span></div>`;
  fields.innerHTML = html;

  const iconWrap = document.getElementById('txDetailIconPage');
  iconWrap.style.cssText = `width:64px;height:64px;margin:0 auto 16px;border-radius:20px;display:flex;align-items:center;justify-content:center;background:var(--${tx.icon_class||'gray'}-light);color:var(--${tx.icon_class||'gray'});`;

  showPage('txdetail');
}

function backFromTxDetail() {
  showPage(txDetailBackPage);
}
...
async function renderWallet() {
  ...
  const { data: cards, error } = await sb.from('virtual_cards')
    .select('id,card_type,cardholder_name,card_number,cvv,expiry_month,expiry_year,status,created_at')
    .eq('user_id', currentProfile.id)
    .order('created_at', { ascending: false });

  if (error) {
    console.error('Card load error:', error);
    container.innerHTML = '<div class="empty-cards-state"><strong>Unable to load cards</strong><span>Please try again in a moment.</span></div>';
    return;
  }

  const { data: requests } = await sb.from('card_requests')
    .select('id,card_type,fee_amount,fee_currency,status,created_at')
    .eq('user_id', currentProfile.id)
    .in('status', ['awaiting_payment','payment_pending'])
    .order('created_at', { ascending: false });

  const pending = (requests || [])[0];
  activeCardRequestId = pending?.id || null;
  ...
  const cardsHtml = (cards || []).map(card => {
    ...
    const type = String(card.card_type || 'VISA').toUpperCase();
    return `
      <article class="crest-virtual-card ${type === 'MASTERCARD' ? 'mastercard-card' : 'visa-card'}">
        <div class="crest-card-top"><span class="crest-card-brand">CREST</span><span class="crest-card-network">${type === 'MASTERCARD' ? '<span class="mini-mc"><i></i><i></i></span> MASTERCARD' : 'VISA'}</span></div>
        <div class="crest-card-chip"><span></span><span></span><span></span></div>
        <div class="crest-card-contactless">⌁</div>
        <div class="crest-card-number">${masked}</div>
        <div class="crest-card-bottom">
          <div><small>CARDHOLDER</small><strong>${escapeHtml(card.cardholder_name || currentProfile.name)}</strong></div>
          <div><small>EXPIRES</small><strong>${expiry}</strong></div>
        </div>
        <div class="crest-card-balance"><strong>${fmtMoney(currentProfile.balance, currentProfile.currency)}</strong></div>
        <button class="crest-card-details-btn" type="button" aria-label="View card details" title="View card details" onclick='showCardDetails(${JSON.stringify({id:card.id,number:card.card_number,cvv:card.cvv,expiry,type})})'><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="2.8"/></svg></button>
      </article>`;
  }).join('');

  const pendingHtml = pending ? `
    <div class="card-pending-box">
      <div class="card-pending-icon">⌛</div>
      <div><strong>${String(pending.card_type).toUpperCase()} card request pending</strong><span>Complete the fixed &euro;${Number(pending.fee_amount).toFixed(2)} payment and wait for admin review.</span></div>
    </div>` : '';

  container.innerHTML = cardsHtml + pendingHtml + (!cardsHtml && !pendingHtml ? `
    <div class="empty-cards-state">
      <div class="empty-card-icon">▣</div>
      <strong>No Crest cards yet</strong>
      <span>Choose Visa or Mastercard to start your card request.</span>
      <button class="add-card-btn" onclick="openCardRequestModal()">Add your first card</button>
    </div>` : '');
}
...
function openCardRequestModal() {
  if (!currentProfile) return showToast('Please sign in first', 'error');
  if (activeCardRequestId) return showToast('You already have a card request awaiting payment or review', 'info');
  document.getElementById('cardHolderName').value = currentProfile.name || currentUser?.user_metadata?.name || currentUser?.email || '';
  selectedCardType = 'VISA';
  selectCardType('VISA');
  document.getElementById('cardRequestModal').classList.add('active');
}

function closeCardRequestModal() {
  document.getElementById('cardRequestModal')?.classList.remove('active');
}

function selectCardType(type) {
  selectedCardType = String(type).toUpperCase() === 'MASTERCARD' ? 'MASTERCARD' : 'VISA';
  document.querySelectorAll('.card-network-option').forEach(btn => btn.classList.toggle('active', btn.dataset.cardType === selectedCardType));
}

async function continueCardRequest() {
  if (!currentProfile) return showToast('Please sign in first', 'error');
  const fullName = (currentProfile.name || currentUser?.user_metadata?.name || currentUser?.email || '').trim();
  if (!fullName) return showToast('Your profile name is required before ordering a card', 'error');

  await withLoader(async () => {
    const { data: existing } = await sb.from('card_requests').select('id,status').eq('user_id', currentProfile.id).in('status',['awaiting_payment','payment_pending']).maybeSingle();
    if (existing) {
      activeCardRequestId = existing.id;
      throw new Error('You already have a card request awaiting payment or review');
    }

    const pin = (document.getElementById('cardPinInput')?.value || '').trim();
    const confirmPin = (document.getElementById('cardPinConfirmInput')?.value || '').trim();
    if (!/^[0-9]{4,6}$/.test(pin)) throw new Error('Card PIN must be 4-6 digits');
    if (pin !== confirmPin) throw new Error('Card PINs do not match');

    const { data: req, error } = await sb.rpc('create_card_request', {
      p_full_name: fullName,
      p_card_type: selectedCardType,
      p_pin: pin
    });
    if (error) throw error;
    if (!req?.id) throw new Error('Unable to create card request');
    activeCardRequestId = req.id;
  }, 'Preparing card request...');

  closeCardRequestModal();
  showToast('Card request created. Continue with the €200 payment.', 'success');
  openCardFeePayment(activeCardRequestId);
}

function openCardFeePayment(requestId) {
  activeCardRequestId = requestId;
  showPage('deposit');
  const title = document.querySelector('#page-deposit .deposit-title');
  const subtitle = document.querySelector('#page-deposit .deposit-subtitle');
  const amount = document.getElementById('cryptoAmount');
  const txid = document.getElementById('cryptoTxid');
  const button = document.querySelector('#page-deposit .crypto-deposit-btn');
  const info = document.querySelector('#page-deposit .crypto-deposit-info');
  if (title) title.textContent = 'Card Fee Payment';
  if (subtitle) subtitle.textContent = 'Complete the fixed €200 card issuance payment using the existing payment and proof workflow.';
  if (amount) { amount.value = CARD_FEE_EUR.toFixed(2); amount.readOnly = true; amount.step = '0.01'; }
  const amountLabel = document.querySelector('#page-deposit label[for="cryptoAmount"]') || document.querySelector('#page-deposit #cryptoAmount')?.closest('.form-group')?.querySelector('.form-label');
  if (amountLabel) amountLabel.textContent = 'Card Issuance Fee (EUR)';
  if (txid) txid.placeholder = 'Payment reference / TXID';
  if (button) button.lastChild.textContent = ' I\'ve Made the €200 Payment';
  if (info) info.querySelector('p').textContent = 'This payment is for card issuance and will not be added to your account balance. Admin approval is required before your card is created.';
  document.getElementById('cryptoSelect').disabled = false;
}
...
function resetDepositMode() {
  ...
  const title = document.querySelector('#page-deposit .deposit-title');
  const subtitle = document.querySelector('#page-deposit .deposit-subtitle');
  if (amount) { amount.readOnly = false; amount.value = ''; amount.step = '0.000001'; }
  const amountLabel = document.querySelector('#page-deposit #cryptoAmount')?.closest('.form-group')?.querySelector('.form-label');
  if (amountLabel) amountLabel.textContent = 'Amount Deposited (in selected crypto)';
  if (title) title.textContent = 'Crypto Deposit';
  if (subtitle) subtitle.textContent = 'Deposit funds instantly using cryptocurrency. Your deposit will be reviewed by our team.';
}
...
async function showCardDetails(card) {
  ...
  if (!modal) return;
  const numberEl = document.getElementById('detailCardNumber');
  const cvvEl = document.getElementById('detailCardCvv');
  const expiryEl = document.getElementById('detailCardExpiry');
  const typeEl = document.getElementById('detailCardType');
  if (numberEl) numberEl.textContent = formatCardNumber(card.number);
