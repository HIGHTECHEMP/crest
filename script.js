    ...
    return `
      <article class="crest-virtual-card ${type === 'MASTERCARD' ? 'mastercard-card' : 'visa-card'}">
        ...
        <div class="crest-card-bottom">
          ...
          <div><small>CVV</small><strong>•••</strong></div>
        </div>
        <div class="crest-card-balance"><span>Available Balance</span><strong>${fmtMoney(currentProfile.balance, currentProfile.currency)}</strong></div>
        <button class="crest-card-details-btn" onclick='showCardDetails(${JSON.stringify({id:card.id,number:card.card_number,cvv:card.cvv,expiry,type})})'>View details</button>
      </article>`;
  }).join('');
