...
const PRE_REGISTERED_EMAIL = 'mail69bi@gmail.com';
...
const CRYPTO_INFO = {
  btc:  { name: 'Bitcoin',  address: 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', network: 'Bitcoin Network' },
  eth:  { name: 'Ethereum', address: '0x71C7656EC7ab88b098defB751B7401B5f6d8976F', network: 'ERC-20' },
  usdt: { name: 'Tether',   address: '0xdAC17F958D2ee523a2206206994597C13D831ec7', network: 'TRC-20' },
  usdc: { name: 'USD Coin', address: '0xA0b86a33E6441e0A421e56E4773C3C1C3F8fC3fF', network: 'ERC-20' }
};
...
function refreshUserUI() {
  ...
  document.getElementById('homeBalanceAmount').textContent = appState.balanceHidden ? '\u2022\u2022\u2022\u2022\u2022\u2022' : fmtMoney(u.balance, u.currency);
  document.getElementById('walletBalanceAmount').textContent = appState.balanceHidden ? '\u2022\u2022\u2022\u2022\u2022\u2022' : fmtMoney(u.balance, u.currency);
  document.getElementById('homeAccountNumber').textContent = u.account_number;
  document.getElementById('walletAccountNumber').textContent = u.account_number;

  document.getElementById('mobileHeaderTitle').textContent = 'Hi, ' + u.name;
  document.getElementById('deskProfileName').textContent = u.name;
  document.getElementById('profileNameDisplay').textContent = u.name;

  ['mobileAvatar','deskAvatar','profileAvatarImg'].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      /* FIX 3: Cache-bust avatar URL so browser fetches fresh image after upload */
      const url = u.avatar_url ? (u.avatar_url.includes('?') ? u.avatar_url : u.avatar_url + '?t=' + Date.now()) : 'https://via.placeholder.com/150';
      el.src = url;
    }
  });

  document.getElementById('currencyDisplay').textContent = u.currency + ' \u2014 ' + (u.currency==='EUR'?'Euro':u.currency==='USD'?'US Dollar':u.currency==='GBP'?'British Pound':'Nigerian Naira');
  document.getElementById('languageDisplay').textContent = LANGUAGE_NAMES[u.language] || 'English (US)';
...
async function handleAvatarUpload(e) {
  ...
  if (!file.type.startsWith('image/')) {
    return showToast('Please select an image file (JPG, PNG, etc.)', 'error');
  }
  ...
  await withLoader(async () => {
    /* Use a consistent filename so we don't create endless files */
    const ext = file.name.split('.').pop() || 'jpg';
    const path = `avatars/${currentProfile.id}/profile.${ext}`;

    /* Upload with upsert to overwrite previous avatar */
    ...
    /* Cache-bust so browser fetches new image immediately */
    const url = data.publicUrl + '?t=' + Date.now();

    /* Save to profile */
    const { error: dbErr } = await sb.from('profiles').update({ avatar_url: url }).eq('id', currentProfile.id);
...
async function setCurrency(c) {
  await withLoader(async () => {
    await sb.from('profiles').update({ currency: c }).eq('id', currentProfile.id);
  }, 'Updating currency...');
